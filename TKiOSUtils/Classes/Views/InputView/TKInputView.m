...
#define TKInputView_ScreenW    [UIScreen mainScreen].bounds.size.width
#define TKInputView_ScreenH    [UIScreen mainScreen].bounds.size.height
#define TKInputView_StyleLarge_LRSpace 10
#define TKInputView_StyleLarge_TBSpace 8
#define TKInputView_StyleDefault_LRSpace 5
#define TKInputView_StyleDefault_TBSpace 5
#define TKInputView_CountLabHeight 20
#define TKInputView_BgViewColor [UIColor colorWithRed:0 green:0 blue:0 alpha:0.3]

#define TKInputView_StyleLarge_Height 170
#define TKInputView_StyleDefault_Height 45

static CGFloat keyboardAnimationDuration = 0.5;

@interface TKInputView ()<UITextViewDelegate,UIGestureRecognizerDelegate>
...
+ (void)showWithStyle:(InputViewStyle)style configurationBlock:(void(^)(TKInputView *inputView))configurationBlock sendBlock:(BOOL(^)(NSString *text))sendBlock{
    TKInputView *inputView = [[TKInputView alloc] initWithStyle:style];
...
- (void)show{
    if([self.delegate respondsToSelector:@selector(TKInputViewWillShow:)]){
        [self.delegate TKInputViewWillShow:self];
    }
    _textView.text = nil;
...
- (void)hide {
    ...
    if([self.delegate respondsToSelector:@selector(TKInputViewWillHide:)]){
        [self.delegate TKInputViewWillHide:self];
    }
...
- (void)setupUI{
    ...
    switch (_style) {
        case InputViewStyleDefault:{
            ...
            _inputView.frame = CGRectMake(0, TKInputView_ScreenH, TKInputView_ScreenW, TKInputView_StyleDefault_Height);
            
            /** StyleDefaultUI */
            CGFloat sendButtonWidth = 45;
            CGFloat sendButtonHeight = _inputView.bounds.size.height -2*TKInputView_StyleDefault_TBSpace;
            _sendButton = [UIButton buttonWithType:UIButtonTypeCustom];
            _sendButton.frame = CGRectMake(TKInputView_ScreenW - TKInputView_StyleDefault_LRSpace - sendButtonWidth, TKInputView_StyleDefault_TBSpace,sendButtonWidth, sendButtonHeight);
            ...
            _textView = [[UITextView alloc] initWithFrame:CGRectMake(TKInputView_StyleDefault_LRSpace, TKInputView_StyleDefault_TBSpace, TKInputView_ScreenW - 3*TKInputView_StyleDefault_LRSpace - sendButtonWidth, self.inputView.bounds.size.height-2*TKInputView_StyleDefault_TBSpace)];
        ...
        case InputViewStyleLarge:{
            ...
            CGFloat height = 170;
            if(TKInputView_ScreenH<=320){
                height = 90;
            }else if(TKInputView_ScreenH<=375){
                height = 140;
            }
            _inputView.frame = CGRectMake(0, TKInputView_ScreenH, TKInputView_ScreenW, height);
            
            /** StyleLargeUI */
            CGFloat sendButtonWidth = 58;
            CGFloat sendButtonHeight = 29;
            _sendButton = [UIButton buttonWithType:UIButtonTypeCustom];
            _sendButton.frame = CGRectMake(TKInputView_ScreenW - TKInputView_StyleLarge_LRSpace - sendButtonWidth, self.inputView.frame.size.height - TKInputView_StyleLarge_TBSpace - sendButtonHeight, sendButtonWidth, sendButtonHeight);
            ...
            _textBgView = [[UIView alloc] initWithFrame:CGRectMake(TKInputView_StyleLarge_LRSpace, TKInputView_StyleLarge_TBSpace, TKInputView_ScreenW-2*TKInputView_StyleLarge_LRSpace, CGRectGetMinY(_sendButton.frame)-2*TKInputView_StyleLarge_TBSpace)];
            _textBgView.backgroundColor = [UIColor groupTableViewBackgroundColor];
            [_inputView addSubview:_textBgView];
            
            _textView = [[UITextView alloc] initWithFrame:CGRectMake(0, 0, _textBgView.bounds.size.width, _textBgView.bounds.size.height-TKInputView_CountLabHeight)];
            ...
            _countLab = [[UILabel alloc] initWithFrame:CGRectMake(0,_textView.bounds.size.height, _textBgView.bounds.size.width-5, TKInputView_CountLabHeight)];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if(object == _textView && [keyPath isEqualToString:@"contentSize"]){
        ...
        CGFloat heightDefault = TKInputView_StyleDefault_Height;
        if(height >= heightDefault){
            [UIView animateWithDuration:0.3 animations:^{
                ...
                //调整sendButton frame
                _sendButton.frame = CGRectMake(TKInputView_ScreenW - TKInputView_StyleDefault_LRSpace - _sendButton.frame.size.width, _inputView.bounds.size.height - _sendButton.bounds.size.height - TKInputView_StyleDefault_TBSpace, _sendButton.bounds.size.width, _sendButton.bounds.size.height);
                //调整textView frame
                _textView.frame = CGRectMake(TKInputView_StyleDefault_LRSpace, TKInputView_StyleDefault_TBSpace, _textView.bounds.size.width, _inputView.bounds.size.height - 2*TKInputView_StyleDefault_TBSpace);
            }];
        } else{
            [UIView animateWithDuration:0.3 animations:^{
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    if ([touch.view isDescendantOfView:_inputView]) { return NO; }
    return YES;
}
...
- (void)keyboardWillAppear:(NSNotification *)noti{
    if(_textView.isFirstResponder){
        ...
        [UIView animateWithDuration:keyboardAnimationDuration animations:^{
            ...
            frame.origin.y = TKInputView_ScreenH - keyboardSize.height - frame.size.height;
            self.inputView.frame = frame;
            self.backgroundColor = TKInputView_BgViewColor;
...
- (void)keyboardWillDisappear:(NSNotification *)noti{
    ...
    if(_textView.isFirstResponder){
        [UIView animateWithDuration:keyboardAnimationDuration animations:^{
            CGRect frame = self.inputView.frame;
            frame.origin.y = TKInputView_ScreenH;
            self.inputView.frame = frame;
            self.backgroundColor = [UIColor clearColor];
